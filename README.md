...
Loads the registered LightGBM model.

Checks the available real-time feature history.

Uses real-time data when sufficient history is available.

Falls back to validated historical data when necessary.

Generates AQI predictions every three hours.

Recursively updates AQI lag and rolling features.

Assigns AQI categories, alerts, and health guidance.
